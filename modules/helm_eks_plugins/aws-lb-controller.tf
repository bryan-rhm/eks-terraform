...
resource "helm_release" "aws_lb_controller" {
  ...
  set {
    ...
    value = module.aws_lb_controller_role[0].iam_role_arn
  }
}
...
module "aws_lb_controller_role" {
  ...
  role_policy_arns              = [aws_iam_policy.aws_lb_controller[0].arn]
...
resource "aws_iam_policy" "aws_lb_controller" {
  ...
  policy      = data.http.aws_alb_controller_policy[0].body
}
